...
`setup` 会先校验 Node.js 版本和系统架构，再删除旧的 `node_modules`，使用 `package-lock.json` 执行 `npm ci --include=optional`，最后验证 Vite/Rolldown 原生绑定是否能正常加载。这样可以避免 Mac 和 Windows 之间复用旧依赖，或 Windows 可选原生依赖安装损坏后继续启动。
...
如果 `npm run dev`、`npm run build`、`npm test` 或 `npm run preview` 启动前发现依赖缺失、原生绑定损坏，会直接提示重新执行 `npm run setup`。
...
```bat
npm run dev -- "C:\Users\hardy\AgentPipeline\outputs\article.md"
```
